...
async def test_analyze_happy_path(client: AsyncClient):
    ...
    response = await client.post(
        "/api/analyze", json={"listing_text": "Python bilen stajyer ariyoruz"}
    )
...
@pytest.mark.asyncio
async def test_analyze_saves_to_job_listings(client: AsyncClient):
    fake_result = {
    ...
    response = await client.post(
        "/api/analyze", json={"listing_text": "Python dev ariyoruz"}
    )
    ...
    assert listing.raw_text == "Python dev ariyoruz"
...
async def test_analyze_agent_error_returns_503(client: AsyncClient):
    ...
    response = await client.post(
        "/api/analyze", json={"listing_text": "bir ilan metni"}
    )
